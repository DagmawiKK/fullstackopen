  ...
  return (
    <>
      ...
      {weather?.current ? (
        ...
        <p>Weather data not available</p> // Fallback message when weather data is not available
      )}
    </>
  );
};
...
const App = () => {
  ...
  const handleShow = (c) => {
    if (show.includes(c.name.common)) {
      setShow(show.filter(name => name !== c.name.common)) // Hide the country if it's already being shown
    } else {
      setShow([...show, c.name.common]) // Show the country if it's not already being shown
    }
  }
